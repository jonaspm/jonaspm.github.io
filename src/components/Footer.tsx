...
function Footer() {
    ...
    const props = {
        innerClass: 'fill-primary hover:fill-primary/60  transition duration-500 ease-in-out hover:scale-125 w-8 h-8'
    }
            ...
            <div className="flex flex-row justify-center items-center flex-wrap gap-8 m-4 px-2 pt-2">
                {
                    socialNetworks.map((socialNetwork, index) => {
                        return (
                            <a key={index} href={socialNetwork.url} target="_blank" aria-label={socialNetwork.name} rel="noreferrer" className="flex flex-column align-center">
